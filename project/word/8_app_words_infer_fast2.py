#!/usr/bin/env python3
# 8_app_words_infer_fast.py (updated)
# Example:
# python ./word/8_app_words_infer_fast.py --model models/word_lstm_aug/best.pt --camera 0 --device cuda --mirror --width 640 --height 360 --T 96 --run_every 3 --thresh 0.6 --smooth_tau 0.6

# python ./word/8_app_words_infer_fast2.py --model models/word_lstm_aug/best.pt --camera 0 --device cuda --mirror --width 640 --height 360 --T 96 --run_every 3 --thresh 0.6 --smooth_tau 0.6 --debug_mode
...
# ---------------------------
# camera thread (unchanged except tiny safety)
# ---------------------------
class CameraThread:
...
def main():
    ...
    ap.add_argument("--debug_mode", action="store_true", help="dump feats/mask to JSON for comparison")
    args = ap.parse_args()
    ...
    # stream mode so it keeps tracking across frames
    ...
    while True:
        ...
        # optional internal resize for speed/consistency (keeps aspect ratio if specified)
        if args.target_res:
            try:
        ...
        # compute velocity using measured dt; fallback to fps_approx if dt invalid
        ...
        # inference block same rhythm as before
        if (len(buf_feats)==args.T) and ((len(buf_feats) % args.run_every)==0):
            ...
            if prob >= args.thresh:
                ...
                # speak (non-blocking) when new label detected or prob increased
                if tts is not None:
                    tts.speak(last_label)
            else:
                last_label, last_prob = "", prob

            # optional debug dump (save features + mask to JSON for compare_features.py)
            if args.debug_mode:
                try:
                    ts = time.strftime("%Y-%m-%dT%H-%M-%S")
                    fname = f"log_script_{ts}.json"
                    with open(fname, "w", encoding="utf-8") as fo:
                        json.dump({"feats_arr": X.tolist(), "mask": M.tolist()}, fo)
                    print("Wrote script debug log:", fname)
                except Exception as e:
                    print("Failed writing script debug log:", e)
