...
(Kept same as your original, with fixes for sequence inference)
"""
...
# --- IMPORTANT FIX: For sequence processing, use Holistic in streaming mode (static_image_mode=False).
# This allows mediapipe to maintain tracking state across frames, which is critical for velocities.
# You can tune model_complexity (0,1,2) for accuracy vs speed. Default set to 1 here.
_holistic_kw_stream = dict(static_image_mode=False, model_complexity=1,
                           refine_face_landmarks=False,
                           min_detection_confidence=0.5, min_tracking_confidence=0.5)

# Keep hand-only processing for single-frame inference (fast)
hands = mp_h.Hands(**_HANDS_KW)

# Holistic instance used for sequence inference (streaming)
...
def _process_frame_safe_hands(rgb_image):
    ...
    with _mp_lock:
        try:
            # mark non-writeable before processing (per mediapipe recommendations)
...
def _process_frame_safe_holistic(rgb_image):
    """
    Streaming-safe holistic processing. The global `hol` instance is created once in streaming mode
    (static_image_mode=False) so it retains track state across consecutive frames.
    """
    global hol
    with _mp_lock:
        try:
            ...
            msg = str(e)
            # If mediapipe graph becomes invalid, re-create but keep streaming mode False
            if "Packet timestamp mismatch" in msg or "Graph has errors" in msg:
                try:
                    hol.close()
                except Exception:
                    pass
                hol = mp_hol.Holistic(static_image_mode=False, model_complexity=_holistic_kw_stream.get("model_complexity", 1),
                                      refine_face_landmarks=False,
                                      min_detection_confidence=0.5, min_tracking_confidence=0.5)
                # try once
                rgb_image.flags.writeable = False
...
# ------------------ API endpoints ------------------ (unchanged main endpoints) ------------------
...
def infer():
    ...
    if word_ts is not None:
        try:
            ...
            out_label = label if prob >= thresh else "<UNKOWN>"
...
def infer_seq():
    ...
    # resize frames to this width x height before mediapipe for speed & consistency
    TARGET_RES = (640, 480)
    ...
    if word_lstm_model is not None:
        ...
        for b64 in frames:
            ...
            if img is None:
                continue
            # Resize for consistent scale and improved speed
            ...
            # compute velocities using first two coords (x,y) but using previous full vector if available
            ...
            if prev_l_full is not None and left.size:
                # prev_l_full and left are 42-d vectors; compare the first two coords
