...
const Library = () => {
  ...
  // simple slugify for word filenames
  ...
  const GestureCard = ({ label, category }: { label: string; category: string }) => {
    let imgPath = "/placeholder.svg";

    if (category === "Alphabet") {
      imgPath = `/alphabets/${label}.jpg`;
    } else if (category === "Number") {
      imgPath = `/numbers/${label}.png`;
    } else if (category === "Word") {
      imgPath = `/words/${slugify(label)}.svg`;
    }
    ...
    return (
      <Card className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 overflow-hidden border-border/50 cursor-pointer">
        ...
        <div className="p-4 bg-card">
          ...
          <p className="text-xs text-muted-foreground text-center mt-1">Tap to view details</p>
        </div>
      </Card>
    );
  };
