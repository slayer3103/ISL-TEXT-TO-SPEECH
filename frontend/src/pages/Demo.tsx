...
export default function Demo() {
  ...
  // committed text (kept in sync via onCommittedChange from StringBuilder)
  const [committedText, setCommittedText] = useState<string>("");

  // suggestions for current prefix
  ...
  // small helper to call builder methods safely
  const callBuilder = (fnName: string, ...args: any[]) => {
    try {
      const cur = stringBuilderRef.current;
      if (!cur || typeof cur[fnName] !== "function") return;
      return cur[fnName](...args);
    } catch (e) {
      console.warn("builder call failed", fnName, e);
    }
  };

  // --- WORD COMPLETION LOGIC ---
  // compute current prefix at caret (word currently selected) and fetch suggestions
    const refreshCompletions = () => {
    try {
      const builder = stringBuilderRef.current;
      const committed = (typeof builder?.getCommitted === "function") ? builder.getCommitted() : committedText || "";
      const cursor = (typeof builder?.getWordCursor === "function") ? builder.getWordCursor() : (committed.trim() ? committed.trim().split(/\s+/).length : 0);
      const words = committed.trim() ? committed.trim().split(/\s+/) : [];
      const prefix = cursor > 0 ? (words[cursor - 1] ?? "") : "";
      const distMap: any = {}; // extend if you want model char probs

      // DEBUG: log caret/prefix
      console.debug("[Demo] refreshCompletions committed:", committed, "cursor:", cursor, "prefix:", prefix);

      const top = suggestionEngine.getTopSuggestions(prefix, distMap, 6) || [];

      // DEBUG: log engine output
      console.debug("[Demo] suggestionEngine.getTopSuggestions ->", top);

      // top may be string[] or objects; normalize
      const asStrings = (top as any[]).map(t => (typeof t === "string" ? t : (t.word || "")).toString().toUpperCase());
      const filtered = asStrings
        .filter(w => w.length >= Math.max(1, prefix.length) && w.length <= 12)
        .slice(0, 5);

      console.debug("[Demo] filtered completions ->", filtered);
      setCompletions(filtered);
    } catch (e) {
      console.warn("refreshCompletions error", e);
      setCompletions([]);
    }
  };


  // refresh completions whenever caret / committed / held changes
  useEffect(() => {
    refreshCompletions();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [committedText, heldPrediction]);

  // convenience: accept a completion (replace selected word or insert)
  const acceptCompletion = (word: string) => {
    ...
    try {
      ...
      if (cursor > 0) {
        ...
        callBuilder("replaceAtCursor", String(word).replace(/\s+$/u, ""));
      } else {
        // if cursor=0 insert at front
        callBuilder("insertLastOutput", String(word).replace(/\s+$/u, ""));
      }
      // after inserting, recompute completions
      setTimeout(refreshCompletions, 50);
    } catch (e) {
      console.warn("acceptCompletion failed", e);
    }
  };
      ...
      <div style={{ display: "flex", gap: 20 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ border: "1px solid #e9eee", borderRadius: 8, padding: 12 }}>
            ...
            {/* Prev / Next caret movement */}
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              <button title="Move caret to previous word" onClick={() => callBuilder("prev")} style={{ flex: 1, padding: 10, borderRadius: 8, background: "#f3f4f6", border: "1px solid #e5e7eb", fontSize: 14 }}>
                ‹ Prev
              </button>
              <button title="Move caret to next word" onClick={() => callBuilder("next")} style={{ flex: 1, padding: 10, borderRadius: 8, background: "#f3f4f6", border: "1px solid #e5e7eb", fontSize: 14 }}>
                Next ›
              </button>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              <button title="Insert held preview at caret" onClick={() => {
                if (!heldPrediction) { alert("No held preview available to insert."); return; }
                const token = String(heldPrediction).replace(/\s+$/u, "");
                if (!token) { alert("Held preview is empty."); return; }
                callBuilder("insertLastOutput", token);
              }} style={{ flex: 1, padding: 10, borderRadius: 8, background: "#e6f6ff", border: "1px solid #cfeefc", color: "#0b63a8", fontWeight: 600 }}>
                ⤓ Insert Preview
              </button>

              <button title="Clear held preview" onClick={() => { setHeldPrediction(""); heldRef.current = null; }} style={{ flex: 1, padding: 10, borderRadius: 8, background: "#ffffff", border: "1px solid #e5e7eb", fontSize: 14 }}>
                Clear Preview
              </button>
            </div>

            {/* Word completion pills */}
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 6 }}>Completions</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {completions.length ? completions.map((c, i) => (
                  <button key={i} onClick={() => acceptCompletion(c)}
                    style={{ padding: "6px 10px", borderRadius: 20, background: "#f3f4f6", border: "1px solid #e5e7eb", cursor: "pointer" }}>
                    {c}
                  </button>
                )) : <div style={{ color: "#9aa0a6", fontSize: 13 }}>No completions</div>}
              </div>
            </div>

            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              <button title="Insert space between words" onClick={() => callBuilder("space")} style={{ flex: 1, padding: 10, borderRadius: 8, background: "#f3f4f6", border: "1px solid #e5e7eb", fontSize: 14 }}>
                Space
              </button>
              <button title="Remove last character" onClick={() => callBuilder("deleteLast")} style={{ flex: 1, padding: 10, borderRadius: 8, background: "#f3f4f6", border: "1px solid #e5e7eb", fontSize: 14 }}>
                ⌫ Backspace
              </button>
            </div>

            <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
              <button title="Undo last action" onClick={() => callBuilder("undo")} style={{ flex: 1, padding: 10, borderRadius: 8, background: "#f3f4f6", border: "1px solid #e5e7eb", fontSize: 14 }}>
                ↺ Undo
              </button>
            ...
            <div>
              <button title="Convert built text to speech (TTS)" onClick={() => {
                const built = typeof stringBuilderRef.current?.getCommitted === "function" ? stringBuilderRef.current.getCommitted() : committedText;
                const finalText = (built || "").trim();
                if (!finalText) { alert("Nothing to speak"); return; }
              ...
              }} style={{ width: "100%", padding: 10, borderRadius: 10, background: "linear-gradient(90deg,#f97316,#8b5cf6)", color: "white", border: "none", fontSize: 15 }}>
                🔊 Speak Text
              </button>
            </div>

            <div style={{ marginTop: 8, fontSize: 12, color: "#6b7280" }}>
              Tip: Use Prev/Next to move the caret between words. Select a completion to replace/insert the word; press Space to separate words.
            </div>
          </div>
